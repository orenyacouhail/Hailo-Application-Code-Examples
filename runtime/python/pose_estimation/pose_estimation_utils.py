...
def data_type2dict(net: str, data_type: str) -> dict:
    """
    initiates a dictionary where the keys are layers names and values are the requested data type
    
    Args:
        net(str) : a string containing a path to hef the file
...
def post_process_and_save_inference_results(
    ...
    score_threshold: int, nms_iou_thresh: int, regression_length: int, strides: list[int] 
) -> None:
    """
    ...
    results = post_process(raw_detections, height, width, class_num, 
...
def post_process(raw_detections: dict, height: int, width: int, class_num: int,
                  max_detections: int ,score_threshold: int, nms_iou_thresh: int, regression_length: int, strides: list[int] ) -> dict:
    """
    ...
    Args:
        ...
        class_num (int): Number of classes.
        max_detections (int): Maximal number of detections per class.
        score_threshold: Confidence threshold for filtering (float).
        nms_iou_thresh (float): IoU threshold for NMS.
        regression_length (int): Maximum regression value for bounding boxes.
        strides (list[int]): Stride values for each prediction scale.  
    ...
    predictions_dict = extract_pose_estimation_results(endnodes, height, width,
                                                        class_num, max_detections, score_threshold,
                                                        nms_iou_thresh, regression_length, strides)
...
def visualize_pose_estimation_result(
    ...
    for detection_box, detection_score, detection_keypoints, detection_keypoints_score in zip(box, score, keypoint, keypoint_score):
...
def extract_pose_estimation_results(
    endnodes: list[np.ndarray], height: int, width: int, 
    class_num: int, max_detections: int, score_threshold: float, 
    nms_iou_thresh: float, regression_length: int, strides: list[int]
) -> dict[str, np.ndarray]:
    """
    ...
    decoded_boxes, decoded_kpts = decoder(raw_boxes, kpts, strides, image_dims, regression_length)
    ...
    nms_res = non_max_suppression(
        predictions, conf_thres=score_threshold, 
        iou_thres=nms_iou_thresh, max_det=max_detections
    )
